...
input, output, and command mode, including `command: sync` for read-only
readiness preflight and `command: verify` with read-only plan mode by default
and explicit `verify-run: "true"` execution.
