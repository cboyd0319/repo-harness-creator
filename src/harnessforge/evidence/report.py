...
def build_report(
    ...
    *,
    ...
    payload = {
        ...
        "skillWiring": readiness_payload["skillWiring"],
...
def format_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Local docs: {payload['index']['summary']['localDocCount']}",
    ...
    lines.extend(
        [
            ...
            "## Skill Wiring",
            "",
            f"- Status: `{payload['skillWiring']['status']}`",
            f"- Skill path: `{payload['skillWiring']['skillPath']}`",
            f"- Reference path: `{payload['skillWiring']['referencePath']}`",
            f"- Instruction routes: {len(payload['skillWiring']['instructionRoutes'])}",
            f"- Warnings: {len(payload['skillWiring']['warnings'])}",
            "",
            "## First-Agent Task",
            "",
