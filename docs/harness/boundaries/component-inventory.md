...
| Workflows | `.github/workflows/ci.yml.disabled` and `.github/workflows/harness-self-heal.yml.disabled` are parked HarnessForge maintainer automation definitions during alpha/pre-release. They are not runnable by GitHub while the `.disabled` suffix is present. Self-healing is repo-local here if intentionally re-enabled later. | The only optional generated workflow is the manual HarnessForge CI scaffold. Target generation must not create self-heal, setup, teardown, branch-push, or pull-request automation. | The composite Action is input-driven. It does not schedule jobs, refresh research, create branches, commit, push, or open pull requests. |
