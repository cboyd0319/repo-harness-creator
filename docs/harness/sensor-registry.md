...
| `PYTHONPATH=src:. python3 -m harnessforge report --target . --json` | Unified report contract | Confirms the read-only report composer emits parseable JSON without running target commands. | HarnessForge maintainer | Replace if report output is folded into release evidence automation. | Report, readiness, audit, index, evidence, or Action changes |
